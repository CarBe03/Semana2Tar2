...
class Clientes
{
    ...
    public function insertar($Nombres, $Direccion, $Telefono, $Cedula, $Correo) //INSERT INTO `clientes`(`idClientes`, `Nombres`, `Direccion`, `Telefono`, `Cedula`, `Correo`)
    {
        try {
            $con = new ClaseConectar();
            $con = $con->ProcedimientoParaConectar();
    ...
    public function actualizar($idClientes,$Nombres, $Direccion, $Telefono, $Cedula, $Correo) //UPDATE `clientes` SET `idClientes`='[value-1]',`Nombres`='[value-2]',`Direccion`='[value-3]',`Telefono`='[value-4]',`Cedula`='[value-5]',`Correo`='[value-6]' WHERE 1
    {
        try {
            $con = new ClaseConectar();
            $con = $con->ProcedimientoParaConectar();
